...
const wchar_t* modelFile = L"assets/Alicia/Alicia/FBX/Alicia_solid_Unity.FBX";
std::vector<SharedStruct::Mesh> meshes; // メッシュの配列
...
std::vector< std::shared_ptr<DescriptorHandle>> materialHandles; // テクスチャ用のハンドル一覧
...
bool SampleScene::Init()
{
	...
	{
		auto size = sizeof(SharedStruct::Vertex) * meshes[i].Vertices.size();
...
void SampleScene::Update()
{
	...
	auto currentTransform = constantBuffer[currentIndex]->GetPtr<SharedStruct::Transform>(); // 現在のフレームの定数バッファを取得
