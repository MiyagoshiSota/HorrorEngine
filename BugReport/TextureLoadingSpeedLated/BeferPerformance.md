...
データ転送にかかる時間は、論理的に以下の3要素の合計で構成される。

**Data Transfer Total Time = CPU Copy + GPU Copy + Sync Overhead** 
...
#### 3. Sync Overhead の理論値

**前提環境**: DirectX 12 Fence同期

* **同期コスト**:
    - コマンド発行およびフェンスシグナル処理にかかるAPIオーバーヘッド。
    - 実効予測値: 0.1 ～ 0.5 ms

#### 結論：理論と現実の比較

理論上の合計時間​ = 0.5∼1.5 (CPU)　+　0.6 (GPU)　+　0.1 (Sync)　≈　1.2∼2.2 ms​
...
- Sync Overhead: 0.1 ～ 0.5 ms（実効予測値）
- Data Transfer Total（理論上の合計）: 1.2 ～ 2.2 ms
...
- **Sync Overhead**: **1.0 ms以下**
  - 理論値上限（0.5 ms）× 2倍 = 1.0 ms

- **データ転送時間（Data Transfer Total）**: **5.0 ms以下**
  - 理論値上限（2.2 ms）× 2倍 ≈ 4.4 ms → 丸めて5.0 ms
  - CPU Copy + GPU Copy + Sync Overhead + その他オーバーヘッドを含む

### Phase 1 評価結果

Phase1Performance.txtの計測結果（107テクスチャ、全て2048x2048）を分析し、目標値との比較を行う。

#### 定量評価

**各ステップのの統計（107テクスチャの計測結果）**

| ステップ | 最小値 | 最大値 | 平均値 | 目標値 | 達成状況 |
|---------|--------|--------|--------|--------|---------|
| **CPU Copy** | 1.037 ms | 2.243 ms | 約1.40 ms | **3.0 ms以下** | ✅ **達成** |
| **GPU Copy** | 0.036 ms | 1.606 ms | 約0.05 ms | **1.5 ms以下** | ✅ **達成** |
| **Sync Overhead** | 1.951 ms | 4.610 ms | 約2.45 ms | **1.0 ms以下** | ❌ **未達成** |
| **Data Transfer (Total)** | 3.225 ms | 6.401 ms | 約3.85 ms | **5.0 ms以下** | ✅ **達成** |

**注**: 
- GPU Copyの最大値（1.606 ms）は最初のテクスチャロード時のコールドスタートによる外れ値。2番目以降は0.036～0.116 msの範囲に収まっている。
- Sync Overheadは目標値（1.0 ms）を大きく超過しており、フェンス同期の待機時間が主因。
...
**GPU Copy（PCIe転送）**
- **実測平均値**: 約0.05 ms（外れ値を除く）
- **目標値**: 1.5 ms以下
- **評価**: ✅ **目標値を達成**。理論値（0.6～0.7 ms）よりも大幅に良好な結果。コールドスタート時（1.606 ms）を除けば、全てのテクスチャで目標値を大幅に下回っている。

**Sync Overhead（フェンス同期オーバーヘッド）**
- **実測平均値**: 約2.45 ms
- **目標値**: 1.0 ms以下
- **評価**: ❌ **目標値を未達成**。目標値の約2.5倍となっており、フェンス同期の待機時間が大きい。

**Data Transfer (Total)**
- **実測平均値**: 約3.85 ms
- **目標値**: 5.0 ms以下
- **評価**: ✅ **目標値を達成**。平均値は目標値内に収まっているが、最大値（6.401 ms）は目標値を超過している。
- **内訳**: CPU Copy（1.40 ms）+ GPU Copy（0.05 ms）+ Sync Overhead（2.45 ms）≈ 3.90 ms
...
- Data Transfer (Total): 3.225 ms ～ 6.401 ms（平均約3.85 ms）
- Total: 17.415 ms ～ 148.291 ms（平均約32.5 ms）

**改善効果**:
- **Data Transfer**: 約**247倍**の高速化（950 ms → 3.85 ms）
- **Total**: 約**30倍**の高速化（980 ms → 32.5 ms）
