...
if (-not (Test-Path "packages\AssimpCpp.5.0.1.6")) {
    # packages.config は msbuild -t:restore で入らない場合がある
...
Copy-Item "soloud20200207\lib\soloud_static.lib" "external\soloud\lib\x64\Debug\" -Force
& $msbuild $slProj -p:Configuration=Release -p:Platform=x64 -v:m
Copy-Item "soloud20200207\lib\soloud_static.lib" "external\soloud\lib\x64\Release\" -Force
